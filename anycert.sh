...
banner() {
  ...
  echo ""
  echo "  用途："
  echo "    自動偵測本機 IP 與 FQDN，產生 10 年自簽 Root CA 與"
  echo "    伺服器憑證，並安裝到指定的自託管服務中（如 PVE、Nginx 等）。"
  echo "    讓瀏覽器在內網可以安全連線並顯示鎖頭。"
  echo ""
  echo "  使用方法："
  echo "    bash anycert.sh        # 安裝憑證"
  echo "    bash anycert.sh -u     # 解除安裝並還原備份"
  echo ""
}
...
detect_listening_ports() {
  ...
  if command -v ss &>/dev/null; then
    ports=$(ss -tln | awk 'NR>1 {print $4}' | awk -F: '{print $NF}' | sort -n -u | xargs 2>/dev/null)
  elif command -v netstat &>/dev/null; then
    ports=$(netstat -an | grep -i listen | awk '{print $4}' | awk -F. '{print $NF}' | sort -n -u | xargs 2>/dev/null)
  elif command -v lsof &>/dev/null; then
    ports=$(lsof -iTCP -sTCP:LISTEN -P -n | awk 'NR>1 {split($9, a, ":"); print a[length(a)]}' | sort -n -u | xargs 2>/dev/null)
  fi
  
  local filtered=""
  for p in $ports; do
    if [[ "$p" -gt 80 && "$p" -ne 135 && "$p" -ne 445 && "$p" -ne 5357 && "$p" -ne 111 ]]; then
      filtered="$filtered $p"
    fi
  done
  echo "$filtered" | xargs
    ...
    if [[ -n "$EXISTING_PORTS" ]]; then
      ...
      if [[ "$port_opt" == "1" ]]; then
        ...
        if [[ -n "$DETECTED_PORTS" ]]; then
          echo -e "  [INFO] Detected active local TCP ports: ${GREEN}${DETECTED_PORTS}${RESET}"
        fi
        echo -e "  Nginx will map new ports to ${CYAN}Port + 10000${RESET} under HTTPS (e.g. 3000 -> 13000)."
        echo -e "  For ports >= 55536, it maps to ${CYAN}Port - 10000${RESET} (e.g. 60000 -> 50000) to fit in TCP range (1-65535)."
        read -rp "  Enter new HTTP ports to add (space-separated): " NEW_PORTS
        ...
        echo -e "  Nginx will map these ports to ${CYAN}Port + 10000${RESET} under HTTPS (e.g. 3000 -> 13000)."
        echo -e "  For ports >= 55536, it maps to ${CYAN}Port - 10000${RESET} (e.g. 60000 -> 50000) to fit in TCP range (1-65535)."
        read -rp "  Enter new list of HTTP ports (space-separated): " NEW_LIST
      ...
      echo -e "  Nginx will map these ports to ${CYAN}Port + 10000${RESET} under HTTPS (e.g. 3000 -> 13000)."
      echo -e "  For ports >= 55536, it maps to ${CYAN}Port - 10000${RESET} (e.g. 60000 -> 50000) to fit in TCP range (1-65535)."
      read -rp "  Enter the HTTP ports of your local services to wrap in SSL (space-separated, e.g. 3000 6000 11434): " PROXY_PORTS
    fi
    
    while [[ -z "$PROXY_PORTS" ]]; do
      warn "You must specify at least one port for proxying!"
      read -rp "  Enter ports: " PROXY_PORTS
    done
    echo ""
  fi
}
...
generate_server_cert() {
  ...
  san_conf=$(mktemp)
  cat > "$san_conf" <<EOF
subjectAltName=DNS:${SERVER_FQDN},IP:${SERVER_IP}
...
verify_cert() {
  ...
  if echo "$san_line" | grep -q "IP Address:${SERVER_IP}"; then
...
install_cert() {
  ...
  if [[ "$PROFILE" == "pve" ]]; then
    ...
    for P in ${PROXY_PORTS}; do
      ...
      # Collision resolver loop
      while true; do
        local collided=0
        # Check against input HTTP ports
        ...
        # Check against already assigned SSL ports
        for AP in "${ASSIGNED_SSL_PORTS[@]}"; do
          if [[ $SSL_P -eq $AP ]]; then
            collided=1
            break
          fi
